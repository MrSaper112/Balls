export class PlaneManager {
  ...
  type: number;
  listOfDivs: Array<HTMLDivElement>;
  mainData: { startIndex: number; endIndex: number; numberOfSteps: number };
  wallArray: {
    top: Array<number>;
    bottom: Array<number>;
    left: Array<number>;
  ...
  changeStartStop(div: HTMLDivElement) {
  ...
  calculateBestTrack() {
    let mapOfDivInsidePlane: HTMLCollection = this.plane.children;
    console.log(mapOfDivInsidePlane);
    if (this.mainData.numberOfSteps == 0) {
      let indexTolookUp: number = this.mainData.startIndex;
